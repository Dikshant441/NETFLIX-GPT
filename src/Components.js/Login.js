  ...
  return (
    <div>
      ...
      <form className="w-3/12 p-12 my-36 mx-auto right-0 left-0 absolute bg-black rounded-md bg-opacity-80">
        <h1 className="text-white text-3xl font-medium py-8 ">{isSignInForm ? "Sign In" : "Sign Up" }</h1>
        {!isSignInForm && <input
          type="text"
          placeholder="Name"
        ...
        <p className="text-white cursor-pointer" onClick={toggleSignInForm}>{isSignInForm ? "New to Netflix? Sign Up Now" : "Alredy registered? Sign In Now." }</p>
      </form>
    </div>
  );
};
